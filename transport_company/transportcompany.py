...
class TransportCompany:
    ...
    def create_itineraries(self, wtf: bool = False):
        ...
        while not self.stationnet.is_empty():
            # create driver and itinerary list for them
            driver = Driver(k, self.stationnet)
            while driver.work_time_remaining > dt.timedelta() and driver.clock <= dt.timedelta(days=1) and\
                ...
                # check if better to wait here or travel to the closest station with available package (waiting preferred)
                else:
                    ...
                    for stat_id, distance in driver.current_station.distances.items():  # distances are sorted nondecreasing
                        ...
                        if stat.will_have_available_packages(driver.clock + dt.timedelta(minutes=distance)):
                            ...
                            travel_time = min(
                                stat.when_next_package() - driver.clock, travel_time)
                            # stat = None
                    ...
                    if wait_time <= travel_time:
                        ...
                        if driver.itinerary:  # if it's not driver's first action
                            driver.itinerary.append(
                                f"[{driver.clock_print()}]: WAIT at station {driver.current_station.id} for {wait_time.seconds // 60} minutes")
                            driver.pass_time(wait_time.seconds // 60)
                            driver.current_station.update_packages(
                                driver.clock)
                        else:
                        ...
                        if not driver.itinerary:
                            driver.itinerary.append(
                                f"[{driver.clock_print()}]: START work at station {driver.current_station.get_id()}")
                        # print("*driver travels elsewhere*")
                        driver.itinerary.append(
                            f"[{driver.clock_print()}]: TRAVEL to station {stat.id} from {driver.current_station.get_id()}")
                        driver.travel_to(stat.id)
                        driver.itinerary.append(
                            f"[{driver.clock_print()}]: ARRIVE at station {driver.current_station.get_id()}")
            ...
            driver.itinerary.append(
                f"[{driver.clock_print()}]: FINISH work at station {driver.current_station.id}")
            total_worktime = dt.timedelta(hours=8) - driver.work_time_remaining
            driver.itinerary.append(
                f"Total work time: {total_worktime.seconds // 3600} hours and {total_worktime.seconds % 3600 // 60} minutes")
            driver.itinerary.append(
                f"Number of packages delivered: {driver.packages_delivered}")
